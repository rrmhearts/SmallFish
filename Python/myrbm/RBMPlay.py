...
class RBM(object):
    ...
    def hidden_sample(self, visible, bias=0.):
        ...
        #sample = np.array([ ((sample[(i-1)%len(sample)] + sample[(i+1)%len(sample)] ) % 2 + sample[i] ) % 2 for i in range(len(sample)) ])
...
#            hidden = bernoulli(exp)
            #hidden = np.array([ ((hidden[(i-1)%len(hidden)] + hidden[(i+1)%len(hidden)] ) % 2 + hidden[i] ) % 2 for i in range(len(hidden)) ])
            ...
            if np.isnan(exp).any():
                ...
                exp[np.isnan(exp)] = 0
                #raise NaNError(exp)
            yield visible, exp #hidden #exp
            # Ryan experiment - push neighbors away from each other


            # end Ryan experiment
            visible = self.visible_expectation(bernoulli(exp))#bernoulli(hidden))
    ...
    def get_reconstruction_error(self, visible):
        ...
        if len(visible.shape) == 3 and visible.shape[0] != 0:
            for i in range(visible.shape[0]):
                ...
                ''' LIKE Error measure in evolutionary system!'''
                difference = rvisible - visible[i]
                cerr = np.sum(difference*difference)
            ...
            rerror_eu = np.sum(difference*difference)
            
            pos = visible * rvisible
            percent_pos = np.sum(pos)*1.0/np.sum(visible)
            
            neg = (1-visible[i]) * (1-rvisible)
            percent_neg = np.sum(neg)*1.0/np.sum(1-visible)
            rerror += (1 - percent_neg*percent_pos) + np.sum((1-visible)*rvisible)*1.0/np.sum(1-visible)
        #if random.randint(0, 10) == 1:
            #print "Euclidean Error: ", rerror_eu
        return rerror
    ...
    def get_avg_error(self, rerror=-1):
        ...
        if math.isnan(rerror):
            ...
            self.error = .9*self.error+.1*rerror
        return self.error
...
class Trainer(object):
    ...
    def apply_gradients(self, weights, visible, hidden, learning_rate=0.2):
        ...
        def update(name, g, _g, l2=0):
            ...
            # Set weights between this gradient and previous gradient
            g *= 1 - self.momentum
            ...
            # Update Trainer weights (previous)
            _g[:] = g

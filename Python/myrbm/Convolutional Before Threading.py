...
##     # Binary True works, binary false does not work
    ...
    def pooled_to_hidden(self, pool):
        ...
        # For each dimension
        dims = []
        for i in range(len(self._pool_shape)):
            dims.append( pool.shape[i] * self._pool_shape[i] )
        hidden = np.zeros(dims)
    ...
    def hidden_expectation(self, visible, bias=0.):
        ...
        activation = np.array([
            convolve(visible, self.weights[k, ::-1, ::-1], 'valid') # might need softmax
            for k in range(self.num_filters)]).T + self.hid_bias + bias
        act = activation.T
        ...
        maxvalue = act.max()
        dtype = type(maxvalue)
        larg_pos_val = 0.72*np.log(np.finfo(dtype).max)
        if maxvalue > larg_pos_val:
            act[act > larg_pos_val] = larg_pos_val
        act = np.exp(act)
        expectation = act / (1. + self._pool(act) )
        #print bernoulli(expectation)
        '''if act.max() < np.log(np.finfo(dtype).max 
                / (self._pool_shape[0]*self._pool_shape[1]) -1):
            expectation = np.exp(act) / (1. + self._pool(np.exp(act)) )
        else: #except OverflowError:
            act = act / act.max() * np.log(np.finfo(dtype).max 
                / (self._pool_shape[0]*self._pool_shape[1]) -1)
            expectation = np.exp(act) / (1. + self._pool(np.exp(act)) )'''
        ...
        #print type(act.max())
        # Stable return. Prevents going to infinity
        #act /= act.max()
        return expectation
        
        # working line
        #return 1 / (1. + self._pool(np.exp(-act)) )
        '''  Forces zeros
        submax = act-act.max() print submax softe = np.exp(submax) print softe 
        return softe / (np.exp(-act.max()) + self._pool(softe) ) '''
...
class ConvolutionalTrainer(Trainer):
    ...
    def calculate_gradients(self, visible, cdk=1):
        ...
        b = a / s
        avg = str(b)
        #avg = str(sum(hid) / hid.size())
        print 'h0 avg: ', avg, ' ', str(a), ' ', str(s)
        #print self.rbm.hidden_expectation(visible).mean(axis=-1).mean(axis=-1)
        print gh.mean(), '    ',  gv.mean(), gw.mean()
        print gh.shape, '    ',  gv.shape, gw.shape

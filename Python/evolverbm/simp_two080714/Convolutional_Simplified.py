...
#        ''' Generate a gene at (x,y) '''
        return [self.amplitude, x, y, .95, .95, 0]
...
#                self.weights[i, self.weights[i]>.06] = .06
    ...
    def hidden_expectation(self, visible, bias=0.):
        ...
        '''
        self.target_sparsity -= .0001*self.target_sparsity
        self.target_sparsity += .0001*np.sum(visible)*1.0/visible.size/self.num_filters
...
#                print "OOB! ", aChild.genome[i][g][2]
    ...
    def _insertDeleteMutate(self, aChild, numberInserts, numberDeletes):
        ...
        for i in range (numberInserts):
            ...
            probMat = (aChild.weights[filt] + .01 - np.min(aChild.weights[filt])) /\
                      (np.max(aChild.weights[filt]) - np.min(aChild.weights[filt]))
            probMat = gaussian_filter(probMat, 5) + gaussian_filter(probMat, 3) - gaussian_filter(probMat, 4)
    ...
    def removeDuplicates(self, data = None):
        ...
        while i < len(self.population):
            ...
            while j < len(self.population):
                ...
                if curr_diversity > self.diversity_cutoff+1.5*self.deviation :

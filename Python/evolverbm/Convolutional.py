...
class ConvolutionalEvolution(RBM):
    ...
    def __init__(self, num_filters, filter_shape, pool_shape, binary=True, num_gaussians=5):
        ...
        ''' Visible bias'''
        self.vis_bias = np.zeros(1)-3 #rng.randn(1)   # Scalar visual bias
        ...
        for i in range(len(self.weights)):
            ...
            for x in range(len(self.weights[i])):
                for y in range(len(self.weights[i][x])):
                    a[x][y] *= (1+((x-len(self.weights[i])*1.0/2)**2 +\
                                   (y-len(self.weights[i][x])*1.0/2)**2)/100.0)
            a = a / sum(a)
            b = self.weights[(i+1)%len(self.weights)] / sum(self.weights[(i+1)%len(self.weights)])
            self.filtercorrelation += float(corr(a, b, 'valid'))
    def generateGene(self): # for a filter
        ''' Generate a gene consisting of random initialized values'''
        angle = random.randint(0,180)
        return [abs(random.normalvariate(.01,.005)), random.randint(3,self.weights_size[1]-3), 
                                           random.randint(3,self.weights_size[2]-3),
                      math.exp(random.normalvariate(0.3,1)), math.exp(random.normalvariate(0,1)), angle]
    ...
    def calculateFitness(self, data):
        ...
        for i in self.genome:
            numgaussians += len(i)
        self.fitness = - .9 * error  + .1 * self.fitness#- error*numgaussians/10000000
        
        self.fitness -= .9 * random.uniform(0,1) * self.filtercorrelation
    ...
    def _gaussianMutate(self, aChild, number ): #gmutateprob=.7): #number = None):
        ...
        for j in range (0, number): #len(self.population)):
            ...
            if random.randint(0,3) == 1:
                ...
                sc = random.normalvariate(.0,.1) / scale_den
                #/mean_scale/1000
                if sc > 5 or aChild.genome[i][g][0]+sc >5 or aChild.genome[i][g][0]+sc < -1:
            ...
            while aChild.genelocations[i, aChild.genome[i][g][1], aChild.genome[i][g][2]] is True and (mx != 0 or my != 0):
                if random.randint(0,1) is 0 and aChild.genome[i][g][1] > 3 and aChild.genome[i][g][1] < aChild.weights_size[1]-4:
                    aChild.genome[i][g][1] += mx # / abs(mx))
                elif aChild.genome[i][g][2] > 3 and aChild.genome[i][g][2] < aChild.weights_size[2]-4:
                    aChild.genome[i][g][2] += my #int(my / abs(mx))
                else:
                    break
            ...
            if random.randint(0,4) == 1:
                ...
                if total_var < .5:
                    offset += (1-total_var)/2
                elif total_var > 5:
                    offset -= .2
                va = random.normalvariate(offset+.2,.1) #math.exp(random.normalvariate(offset+.2,.1))
                if va < 0:
                    va -= .1
                elif va > 0:
                    va += .1
                aChild.genome[i][g][3] += va
                aChild.genome[i][g][3] = round(aChild.genome[i][g][3], 1)
                if aChild.genome[i][g][3] <= 0:
                    aChild.genome[i][g][3] = .5
            if random.randint(0,4) == 1:
                offset = 0
                total_var = aChild.genome[i][g][3] + aChild.genome[i][g][4]
                if total_var < .5:
                    ...
                    offset -= .2
                va = random.normalvariate(offset+0,.1) #math.exp(random.normalvariate(offset+0,.5))
                if va < 0:
                    va -= .1
                elif va > 0:
                    va += .1
                aChild.genome[i][g][4] += va 
                aChild.genome[i][g][4] = round(aChild.genome[i][g][4], 1)
                if aChild.genome[i][g][4] <= 0:
                    aChild.genome[i][g][4] = .3
            ...
            if random.randint(0,4) == 1:
                ...
                if an < 0:
                    ...
                    an += 5
                aChild.genome[i][g][5] += round(an, 0) #+ .01*devotion*(g_angle - aChild.genome[i][g][5]) )
                aChild.genome[i][g][5] %= 180
            #if random.randint(0,3) != 1:           
            # Always move the bias terms.. there is one best.
            if random.randint(0,1) == 1:
                aChild.hid_bias = .6*aChild.hid_bias + rng.randn(aChild.num_filters)-.5\
                                 + .4*self.best_hid_bias
            else:
                aChild.vis_bias = .6*aChild.vis_bias + rng.randn(1)-.5 + .4*self.best_vis_bias
...
#                c1.genome[:filt_start] = c2.genome[:filt_start]
    ...
    def _scaleMutate(self, aChild, number):
        ''' Mutate scale by random amount. Currently unused '''
...
#        self.population.insert(0, best_fit_individual)
    ...
    def learn(self, visible, children_multiplier = None):#, learning_rate=0.2, cdk=1):
        ...
        self.mutate(math.ceil(self.children_multiplier*self.pop_size), .45, .21, .18, .16)#self.population)
...
#    r = ConvolutionalEvolution(rLayers, [15,15], [2,2], False) #ConvolutionalEvolution(2, [3, 3], [2, 2])
    ...
    t = EvolveTrainer(50, 6, rLayers, 3, num_gaussians=30, mutation_prob = .7)
#    

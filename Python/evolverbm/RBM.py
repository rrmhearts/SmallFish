...
#        return bernoulli( self.hidden_expectation(visible))
        return self.hidden_expectation(visible) > self.bernoul
...
#            return bernoulli( self.visible_expectation(bernoulli(hidden)))
            return self.visible_expectation(hidden > self.bernoul) > self.bernoul
#        return self.visible_expectation(bernoulli(hidden))
        return self.visible_expectation(hidden > self.bernoul)
...
#            hidden = bernoulli(hidden)
            hidden = hidden > self.bernoul #bernoulli(hidden)
#            hidden = hidden > .4
            # Error checking
            ...
            visible = self.visible_expectation(hidden > self.bernoul) #bernoulli(hidden)) #hidden > .5)
#            visible = self.visible_expectation(bernoulli(hidden))
    ...
    def get_reconstruction_error(self, visible):
        ...
        rerror = 0
        rerror_eu = 0
#        print "get_recon_error"
        if len(visible.shape) == 3 and visible.shape[0] > 0:
#            print "HERE: ", visible[i].shape
...
#                rvisible = (rvisible - np.min(rvisible)) / (np.max(rvisible)- np.min(rvisible) + .0000001)
                ''' EUclidean Distance'''
...
#                rerror_eu = rerror_eu + np.sqrt(cerr)
...
#                pos = np.logical_and(visible[i], rvisible)
                pos = visible[i] * rvisible
                percent_pos = np.sum(pos)*1.0/np.sum(visible[i])
                
#                neg = np.logical_and(1-visible[i], 1-rvisible)
                neg = (1-visible[i]) * (1-rvisible)
                percent_neg = np.sum(neg)*1.0/np.sum(1-visible[i])
#                rerror += (1 - percent_neg*percent_pos) + np.sum(np.logical_and(1-visible[i], rvisible))*1.0/np.sum(1-rvisible)
                rerror += (1 - percent_neg*percent_pos) + np.sum((1-visible[i])*rvisible)*1.0/np.sum(1-visible[i])
#            rerror_eu = rerror_eu / visible.shape[0]
            rerror = rerror / visible.shape[0]
            
#            rerror -= rerror/10
#            rerror += rerror_eu/10
        elif visible.shape[0] > 0 and len(visible.shape) < 3:
#            print "HERE 2: ", visible[i].shape

            rvisible = self.reconstruct(visible,2)
            difference = rvisible - visible
            rerror_eu = np.sqrt(np.sum(difference*difference)/(visible.shape[0]*visible.shape[1]))
...
#            visible[visible <= .5] = 0
            ...
            rerror += (1 - percent_neg*percent_pos) + np.sum(np.logical_and(1-visible, rvisible))*1.0/np.sum(1-visible)
            
#            rerror -= rerror/2
#            # Favor traditional error a little bit..
#            rerror += rerror_eu/2
        else:
            print "error xxx"
        return rerror

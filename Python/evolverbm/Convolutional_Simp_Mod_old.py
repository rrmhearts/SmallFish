...
class ConvolutionalEvolution(RBM):
    ...
    def __init__(self, num_filters, filter_shape, pool_shape, binary=True, num_gaussians=5, best_guy=None):
        ...
        self.target_sparsity = .1 / num_filters * ( .1 * 28 / filter_shape[0]) #3 #.05
        self.entropy = 26
        self.amplitude = random.uniform(0,1)#1.0 / num_filters * random.uniform(0,1)
        self.overhidden = 2
        self.weights_binary = gaussian_filter(np.random.rand(*self.weights_size),1) < \
...
#        self.hid_bias = rng.randn(num_filters)   # Scalar bias for each filter
        ...
        if best_guy is not None:
            ...
            self.weights_binary = gaussian_filter(np.random.rand(*self.weights_size),1) 
            self.weights_binary = self.weights < gaussian_filter(self.weights, 2) \
                        + (best_guy.weights_binary.mean()-.5)
...
#            self.entropy += self.entropyF(self.weights[i].sum(axis=0))+self.entropyF(self.weights[i].sum(axis=-1))
    ...
    def calculateFitness(self, data):
        ...
        self.fitness = 1000* (1.0 - self.error)

        for i in range(len(self.weights)):
            self.fitness -= 100*(self.weights[i].mean() == 0)
#        self.sparsity = 0
        self.sparsity *= .9
        temp_hidden = 0
        for visible in data:
            hidden = self.hidden_sample(visible)
            self.sparsity += .1*(self.target_sparsity - hidden.mean(axis=-1).mean(axis=-1)) / len(data)#axis=-1).mean(axis=-1)
#            self.sparsity /= 2
#            if hidden.mean() > self.target_sparsity + 100*abs(self.sparsity.mean()):
#                self.hid_bias += 40*self.sparsity
#            elif hidden.mean() > self.target_sparsity + 50*abs(self.sparsity.mean()):
#                self.hid_bias += 20*self.sparsity
#            elif hidden.mean() > self.target_sparsity + 30*abs(self.sparsity.mean()):
#                self.hid_bias += 15*self.sparsity
#            elif hidden.mean() > self.target_sparsity + 10*abs(self.sparsity.mean()):
#                self.hid_bias += 10*self.sparsity
#            elif hidden.mean() > self.target_sparsity + 5*abs(self.sparsity.mean()):
#                self.hid_bias += 5*self.sparsity
#            elif hidden.mean() > self.target_sparsity + 2*abs(self.sparsity.mean()):
#                self.hid_bias += 2*self.sparsity
            if hidden.mean() > self.target_sparsity + abs(self.sparsity.mean()):
                self.hid_bias += (hidden.mean()-self.target_sparsity)/abs(self.sparsity.mean())*self.sparsity
            elif hidden.mean() == 0:
                self.hid_bias += 10*abs(self.sparsity)
#                self.fitness -= .5*self.fitness
            elif hidden.mean() < self.target_sparsity:
                self.hid_bias += (self.target_sparsity-hidden.mean())/abs(self.sparsity.mean())*self.sparsity
#                if (self.weights.mean(axis=-1).mean(axis=-1)==0).any():
#                    self.fitness -= .2*self.fitness
            else:
                self.hid_bias += self.sparsity
            ...
            temp_hidden /= (len(hidden)*(len(hidden)-1)*2)
            
#        self.overhidden *= .9
#        self.overhidden += .1 * temp_hidden * 1000
        self.overhidden = temp_hidden * self.error * 1000
            #print "overhidden: ", self.overhidden
#        if self.sparsity.mean() > 0:        
#            self.fitness -= 100*self.sparsity.mean()
        self.fitness -= self.overhidden
#        self.fitness -= 10*abs(self.sparsity.mean())
        self.fitness -= self.calculateEntropy()/100
...
#            if random.randint(0,3) == 1:
            base = (np.max(aChild.weights) - np.min(aChild.weights) + .000001)
            
            ''' CONSTRAINT: Emphasize border regions'''
            probMat = abs ( gaussian_filter(aChild.weights, 1) - gaussian_filter(aChild.weights, 3))
            probMat = (.1*base + probMat - np.min(probMat)) / (1.1*base)
            
            ''' CONSTRAINT: Equalize average mutation amount across filters '''
...
#            old = np.array(aChild.weights_binary)
            ...
            imean = aChild.weights_binary.mean(axis=-1).mean(axis=-1).min()
            uni = np.random.rand(*aChild.weights.shape)
            aChild.weights_binary[np.logical_and(probMat < uni, np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size)), \
                                    p=[48./49, 1./49]).reshape(*aChild.weights_size))] = 0 if imean > aChild.amplitude*5/(aChild.weights_size[1]*aChild.weights_size[2]) else 1
...
#            uni = np.random.rand(*aChild.weights.shape)
            aChild.weights_binary[np.logical_and(probMat > uni, np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size)),\
                                    p=[49./50, 1./50]).reshape(*aChild.weights_size)) ] = 1
...
#            old = np.array(aChild.weights_binary)
            uni = np.random.rand(*aChild.weights.shape)
#            aChild.weights_binary[np.logical_and(posMat > uni, np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size)), p=[39./40, 1./40]).reshape(*aChild.weights_size))] = 1
#            aChild.weights_binary[np.logical_and(uni>.8, np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size)), p=[39./40, 1./40]).reshape(*aChild.weights_size))] = 1
            
            x = [np.random.randint(z) for z in aChild.weights_size]
            maxiter = 50
#            while aChild.weights_binary[x[0], x[1]-1, x[2]-1] == 0 and\
#                  aChild.weights_binary[x[0], x[1],   x[2]-1] == 0 and\
#                  aChild.weights_binary[x[0], x[1]-1, x[2]] == 0 and\
#                  aChild.weights_binary[x[0], (x[1]+1)%aChild.weights.shape[1],\
#                                              (x[2]+1)%aChild.weights.shape[2]] == 0 and\
#                  aChild.weights_binary[x[0], x[1],   (x[2]+1)%aChild.weights.shape[2]] == 0 and\
#                  aChild.weights_binary[x[0], x[1]-1, (x[2]+1)%aChild.weights.shape[2]] == 0 and\
#                  aChild.weights_binary[x[0], (x[1]+1)%aChild.weights.shape[1], x[2]] == 0 and\
#                  aChild.weights_binary[x[0], (x[1]+1)%aChild.weights.shape[1], x[2]-1] == 0 and maxiter > 0:
            while aChild.weights_binary[x[0],x[1],x[2]] and maxiter > 0:
                      x = [np.random.randint(z) for z in aChild.weights_size]
                      maxiter -= 1
            aChild.weights_binary[x[0],x[1],x[2]] = 1
...
#        negMat = (.1*base + probMatTest - np.min(probMatTest)) /\
#                  (.2*base + np.max(probMatTest) - np.min(probMatTest))
...
#            print aChild.weights_binary.shape
            ''' CONSTRAINT: For each filter, only delete if the average is greater than .006. Prevents filter from zeroing out'''
...
#                    np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size[1:])), p=[39./40, 1./40]).reshape(*aChild.weights_size[1:]))] = 0 \
#                    if imean > .006 else 1
#            for i in range(len(aChild.weights_binary)):
            
            x = [np.random.randint(z) for z in aChild.weights_size]
            maxiter = 50
            while not aChild.weights_binary[x[0],x[1],x[2]] and maxiter > 0:
                x = [np.random.randint(z) for z in aChild.weights_size]
                maxiter -= 1
                
            imean = aChild.weights_binary[x[0]].mean()
            aChild.weights_binary[x[0],x[1],x[2]] = 0 if imean > aChild.amplitude*10/(aChild.weights_size[1]*aChild.weights_size[2]) else 1
#            filt = np.random.randint(aChild.weights_size[0])
#            imean = aChild.weights_binary[filt].mean()
#            aChild.weights_binary[filt, np.logical_and(uni[filt]>.8, \
#                np.random.choice([0, 1], size=(reduce(operator.mul, aChild.weights_size[1:])), p=[39./40, 1./40]).reshape(*aChild.weights_size[1:]))] = 0 \
#                if imean > aChild.amplitude*5/(aChild.weights_size[1]*aChild.weights_size[2]) else 1
#            print probMat.min(), " ", probMat.max()
    ...
    def mutate(self, numberChildren, gmutateprob=None, INSmutateprob=None, DELmutateprob=None, cmutateprob=None, scalemutatateprob=None):#, pop):
        ...
        while (len(self.children_pop) < numberChildren):
            ...
            if random.randint(0,9) == 1:
                aChild.vis_bias = .5*aChild.vis_bias + 2*rng.randn(1) + .5*self.best_vis_bias
            if random.randint(0,9) == 1:
                sc = random.normalvariate(.0,.01) 
            ...
            if random.randint(0,10) == 1 and aChild.bernoul < .80:
                aChild.bernoul += .05
            elif random.randint(0,9) == 1 and aChild.bernoul > .2:
                aChild.bernoul -= .05
            else:
                pass
...
#                print "curr div: ", curr_diversity
                if curr_diversity > self.diversity_cutoff+1.7*self.deviation or curr_diversity > .7:
...
#        if self.percent_kept > .8 or self.diversity_cutoff > .95:
#            self.diversity_cutoff -= .2 *(abs(self.diversity_cutoff)*abs(1-self.diversity_cutoff))
#            self.deviation -= .2 *(abs(self.deviation)*abs(1-self.deviation))
#        elif self.percent_kept > .6:
##            self.diversity_cutoff -= .2 *abs(self.diversity_cutoff)
#            self.deviation -= .2 * abs(self.deviation)
#        elif self.percent_kept > .3:
#            self.diversity_cutoff -= .05 *abs(self.diversity_cutoff)
#            self.deviation -= .05 * abs(self.deviation)
        if self.percent_kept < .10 or len(self.population) < self.pop_size:
#            self.diversity_cutoff += .1 * abs(self.diversity_cutoff)
#            self.deviation += .1 * abs(self.deviation)
...
#            self.population[i].fitness -= .001
#            self.population[i].hid_bias += self.population[i].sparsity
#            self.population[i].calculateFitness(data)
...
#                            /abs(self.avg_fitness) > random.normalvariate(0,1))
    ...
    def learn(self, visible, children_multiplier = None):#, learning_rate=0.2, cdk=1):
        ...
        self.mutate(math.ceil(self.children_multiplier*self.pop_size), .1, .3, .3, .3)# .3)#self.population)
